...
| PR12 | Journal comptable consultable ligne à ligne, page de supervision technique (application, base, sauvegardes, tâches, intégrité, services externes, files, dernières erreurs), remise à zéro pré-ouverture (`go_live_reset.py` : listes de tables explicites, sauvegarde obligatoire, une transaction, verrou après coup), tableau du matériel compatible, identifiant de requête affiché en caisse, tests de fumée automatiques après déploiement | livré |
| PR13 | Correctif suivi des paiements terminal SumUp : l'identifiant de transaction rendu par le terminal est celui de SumUp, la caisse passe à « payé » dès que la carte est tapée (et non plus « attente retour du TPE » indéfinie), message d'erreur carte contenu dans son encart | **en cours (cette PR)** |
